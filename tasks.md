...
### 6-1. 마이크로 인터랙션
- [ ] 완료 체크박스 바운스 애니메이션 개선 (spring 효과)
- [ ] 리스트/필터 전환 시 fade 트랜지션
- [ ] 모달 열기 애니메이션 (scale 0.95→1 + opacity 0→1)
- [ ] 모달 닫기 애니메이션 (scale 1→0.95 + opacity 1→0)
- [ ] 리마인더 항목 드래그 앤 드롭 순서 변경 (Backend: `sortOrder` 필드 추가)

### 6-2. 디자인 디테일
- [ ] 사이드바 리스트 항목 더블클릭 → 편집 모달 열기
- [ ] 커스텀 우클릭 컨텍스트 메뉴 컴포넌트 (편집/삭제 옵션)
- [ ] 빈 상태 일러스트레이션 (리스트 없음, 리마인더 없음)
- [ ] AddReminderInput에 날짜/우선순위 빠른 설정 아이콘 버튼 추가

### 6-3. 키보드 단축키
- [ ] `Cmd/Ctrl + N` — 새 리마인더 입력 활성화
- [ ] `Delete` / `Backspace` — 선택된 리마인더 삭제 (confirm)
- [ ] `↑` / `↓` — 리마인더 목록 탐색 (선택 이동)
- [ ] `Escape` — 상세 패널 닫기 / 모달 닫기

### 6-4. 반응형 레이아웃
- [ ] 모바일 (<768px): 사이드바 숨김 + 햄버거 토글 버튼
- [ ] 모바일: 상세 패널을 하단 시트로 전환
- [ ] 태블릿 (768~1024px): 사이드바 축소 모드 (아이콘만 표시)
